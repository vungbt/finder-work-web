...
import { IPhoneCode, IStepItem } from '../providers';
import { Country } from 'countries-and-timezones';

const ProfileInformationStep = dynamic(() => import('../components/create-resume-step-one'), {
  ssr: false
});
...
const ExperienceInformationStep = dynamic(() => import('../components/create-resume-step-two'), {
  ssr: false
});
const EducationInformationStep = dynamic(() => import('../components/create-resume-step-three'), {
  ssr: false
});
const OtherInformationStep = dynamic(() => import('../components/create-resume-step-four'), {
  ssr: false
});
...
export function CreateResumeProvider({ children }: CreateResumeProviderProps) {
  ...
  const reducer = (state: ProfileDetailState, action: Action): ProfileDetailState => {
    switch (action.type) {
      ...
      case CreateResumeActionType.SET_PROFILE_TEMP: {
        ...
        console.log(profileTemp, 123331);
        return {
          ...state,
          profileTemp,
          formData: {
